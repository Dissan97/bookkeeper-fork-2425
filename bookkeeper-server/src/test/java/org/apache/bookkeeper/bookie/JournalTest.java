...
public class JournalTest {
    ...
    public void testScanJournal() {
        ...
        long readBytes = 0;
        try {
            readBytes = this.journal.scanJournal(this.journalID, this.journalPos, this.scanner, false);
        ...
        if (this.expected == Expected.EXACT_BYTES) {
            ...
            if (this.expected == Expected.NPE)
                assertEquals(NullPointerException.class, this.actualException.getClass());
            else if (this.expected == Expected.IOE)
                assertEquals(IOException.class, this.actualException.getClass());
